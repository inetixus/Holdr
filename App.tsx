...
export default function App() {
  ...
  useEffect(() => {
    ...
    Animated.timing(screenEntrance, { toValue: 1, duration: 420, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
...
function AnimatedPressable({ children, style, onPress, ...props }: any) {
  ...
  return (
    <Pressable onPressIn={animateIn} onPressOut={animateOut} onPress={onPress} style={style} {...props}>
...
function AnimatedListItem({ children, index, delay = 60 }: { children: React.ReactNode; index: number; delay?: number }) {
  ...
  useEffect(() => {
    Animated.timing(enter, { toValue: 1, duration: 400, delay: index * delay, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start();
  }, [enter, index, delay]);
...
function Onboarding({ step, floatValue, onNext, onSkip }: { step: number; floatValue: Animated.Value; onNext: () => void; onSkip: () => void }) {
  ...
  return <SafeAreaView style={styles.onboardingSafe}>
    ...
    <View style={styles.onboarding}>
      <View style={styles.onboardingTop}><Text style={styles.wordmark}>holdr<Text style={styles.coral}>.</Text></Text><Pressable onPress={onSkip}><Text style={styles.skip}>Skip</Text></Pressable></View>
      <View style={styles.onboardingMiddle}>
        <View style={styles.blobOne} /><View style={styles.blobTwo} />
        <Animated.View style={[styles.storyCard, { transform: [{ translateY }] }]}>{slide.card === 'receipt' && <ReceiptStory />}{slide.card === 'deadline' && <DeadlineStory />}{slide.card === 'done' && <DoneStory />}</Animated.View>
      </View>
      <View style={styles.onboardingBottom}>
        ...
        <Pressable style={styles.darkButton} onPress={onNext}><Text style={styles.darkButtonText}>{step === 2 ? 'Start holding' : 'Continue'}</Text><Text style={styles.buttonArrow}>{'>'}</Text></Pressable>
      </View>
    </View>
  </SafeAreaView>;
}
...
function AuthScreen({ mode, setMode, onBack, onSuccess }: { mode: 'signup' | 'signin'; setMode: (mode: 'signup' | 'signin') => void; onBack: () => void; onSuccess: (name?: string) => void }) {
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const signingUp = mode === 'signup';
  const submit = () => {
    if (!email.includes('@') || password.length < 4 || (signingUp && !name.trim())) return Alert.alert('Almost there', 'Enter your name, a valid email, and a password with at least 4 characters.');
    onSuccess(signingUp ? name.trim() : email.split('@')[0]);
  };
    ...
    <KeyboardAvoidingView style={styles.authKeyboard} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.authScroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.authForm}>
          ...
          <Pressable style={styles.googleButton} onPress={() => onSuccess('Alex')}><View style={styles.googleMark}><Text style={styles.googleMarkText}>G</Text></View><Text style={styles.googleButtonText}>Continue with Google</Text></Pressable>
        </View>
